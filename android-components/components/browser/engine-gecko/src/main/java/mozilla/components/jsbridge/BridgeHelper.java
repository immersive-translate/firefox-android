...
public class BridgeHelper implements WebViewJavascriptBridge {
    ...
    private static final String BRIDGE_JS = BridgeUtil.JAVA_SCRIPT;
    ...
    private void handlerReturnData(String url) {
        ...
        JsonArray data = BridgeUtil.getDataFromReturnUrl(url);
        if (f != null) {
            JsonObject jsonObject = new JsonObject();
            jsonObject.add("messageArray", data);
            f.onCallBack(jsonObject);
     ...
     */
    private void doSend(String handlerName, JsonObject data, OnBridgeCallback responseCallback) {
        ...
        if (Thread.currentThread() == Looper.getMainLooper().getThread()) {
            loadUrl(BridgeUtil.JS_FETCH_QUEUE_FROM_JAVA, new OnBridgeCallback() {
                ...
                public void onCallBack(JsonObject data) {
                    ...
                    try {
                        list = Message.toArrayList(data.getAsJsonArray("messageArray"));
                    } catch (Exception e) {
                        Log.w(TAG, e);
                        return;
                    }
     ...
     */
    public void callHandler(String handlerName, JsonObject data, OnBridgeCallback callBack) {
    ...
    private void webViewLoadLocalJs() {
        ...
        if (TextUtils.isEmpty(jsContent)) {
            jsContent = BridgeUtil.assetFile2Str(webView.getContext(), BridgeHelper.BRIDGE_JS);
        }
    ...
    @Override
    public void sendToWeb(JsonObject data) {
        sendToWeb(data, (OnBridgeCallback) null);
    }

    @Override
    public void sendToWeb(JsonObject data, OnBridgeCallback responseCallback) {
    ...
    public void sendResponse(JsonObject data, String callbackId) {
        if (!TextUtils.isEmpty(callbackId)) {
            ...
            if (Thread.currentThread() == Looper.getMainLooper().getThread()) {
                ...
                handler.post(() -> dispatchMessage(response));
            }
        }
    }

    @Override
    public void responseFromWeb(JsonObject data, String callbackId) {
