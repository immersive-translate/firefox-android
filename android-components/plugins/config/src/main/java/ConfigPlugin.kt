...
object Config {
    ...
    fun readVersionFromFile(project: Project): String {
        val mozconfig = project.gradle.extensions.extraProperties.get("mozconfig") as Map<*, *>
        val topsrcdir = mozconfig["topsrcdir"] as String
        val candidates = listOf(
            Paths.get(topsrcdir, "mobile/android/version.txt"),
            Paths.get(topsrcdir, "version.txt"),
        )
        val versionFile = candidates
            .map { project.file(it) }
            .firstOrNull { it.isFile }
            ?: throw RuntimeException("Unable to locate version.txt. Checked: ${candidates.joinToString()}")
        return versionFile.useLines { it.firstOrNull() ?: "" }
    }
