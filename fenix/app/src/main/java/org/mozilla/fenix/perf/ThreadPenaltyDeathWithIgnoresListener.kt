...
private const val MIUI_TURBO_SCHED_MONITOR_IMPL_CLASS = "miui.turbosched.TurboSchedMonitorImpl"
private const val MIUI_TYPEFACE_HELPER_CLASS = "miui.util.TypefaceHelper"
private const val MIUI_FONT_VF_UTILS_CLASS = "miui.util.font.VFUtils"
private const val MIUI_FONT_THEME_VF_MANAGER_CLASS = "miui.util.font.ThemeVFManager"
private const val MIUI_TYPEFACE_UTILS_CLASS = "miui.util.TypefaceUtils"
...
class ThreadPenaltyDeathWithIgnoresListener(
    ...
    private fun shouldViolationBeIgnored(violation: Violation): Boolean =
        isSamsungLgEdmStorageProviderStartupViolation(violation) ||
                isXiaomiTurboSchedViolation(violation) ||
                isXiaomiTypefaceViolation(violation) ||
    ...
    private fun isXiaomiTurboSchedViolation(violation: Violation): Boolean {
        // MIUI TurboSched may perform file access checks from Choreographer, which can trigger
        // unavoidable DiskReadViolation on app startup.
        return manufacturerChecker.isXiaomi() &&
                violation.stackTrace.any { it.className == MIUI_TURBO_SCHED_MONITOR_IMPL_CLASS }
    }

    private fun isXiaomiTypefaceViolation(violation: Violation): Boolean {
        // MIUI variable-font replacement may read from disk while resolving Typeface.
        if (!manufacturerChecker.isXiaomi()) return false

        return violation.stackTrace.any {
            it.className == MIUI_TYPEFACE_HELPER_CLASS ||
                    it.className == MIUI_FONT_VF_UTILS_CLASS ||
                    it.className == MIUI_FONT_THEME_VF_MANAGER_CLASS ||
                    it.className == MIUI_TYPEFACE_UTILS_CLASS
        }
    }
