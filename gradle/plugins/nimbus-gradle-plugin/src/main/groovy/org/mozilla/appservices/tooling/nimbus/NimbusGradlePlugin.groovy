...
abstract class ApplicationServicesVersionSource implements ValueSource<String, ApplicationServicesVersionSource.Parameters> {
    ...
    String obtain() {
        ...
        def candidates = [
                new File(topsrcdir, "mobile/android/android-components/plugins/dependencies/src/main/java/ApplicationServices.kt"),
                new File(topsrcdir, "android-components/plugins/dependencies/src/main/java/ApplicationServices.kt"),
        ]
        def appServicesFile = candidates.find { it.isFile() }
        if (appServicesFile == null) {
            throw new GradleException("Could not locate ApplicationServices.kt. Checked: ${candidates*.absolutePath}")
        }
